 ...
 */
@RestController
@RequestMapping("/common/auto-save/v1")
public class AutosaveProducerControllerV1 {

private final ModelMapper modelMapper;
private final AutosaveUseCase autosaveUseCase;

    public AutosaveProducerControllerV1(ModelMapper modelMapper, AutosaveUseCase autosaveUseCase) {
...
//    }
    ...
    public ResponseEntity<?> fetchAutosaveDraftList(@Valid @RequestBody AutosaveCreateRequest request) {
        List<LinkedHashMap<String, Object>> response = autosaveUseCase.fetchAutosaveDraftListWithoutPagination(modelMapper.map(request, AutosaveDomain.class));
        ApiResponse<?> apiResponse = ApiResponse.builder()
                .statusCode(HttpStatus.OK.value())
                .status(HttpStatus.OK.name())
                .message(Constants.FETCH_DRAFT_SUCCESSFULLY)
